...
def classifier(X,y,numComponents = 64, windowSize =7,testRatio = 0.95,dost_ = True,wls_alpha=1.0,wls_lambda=1.0,report=None,batch_size=32,epochs=200,index=1,bands=None,dataset="indian_pines",target_names=None):
    ...
    XPatches, yPatches = createPatches(X_, y, windowSize=windowSize)
    ...
    model = Sequential()
    model.add(Conv2D(2*C1, (5, 5),data_format="channels_last", activation='relu',padding = 'same' ,input_shape=input_shape))
    model.add(Dropout(0.4))
    model.add(Conv2D(C1, (3, 3), data_format="channels_last",activation='relu'))
    model.add(Dropout(0.4))
    model.add(Conv2D(C1//2, (3, 3), data_format="channels_last",activation='relu'))
    model.add(Dropout(0.4))
    model.add(Flatten())
    ...
    # batch_size  = 16
    modelpath = "../models/conv_{}_no_pca_{}_{}_{}_{}_{}_{}.h5".format(dataset,numComponents,windowSize,testRatio,dost_,wls_alpha,wls_lambda,index)
    ...
    def reports (X,y,y_pred,target_names,model):
        ...
        classification = classification_report(y, y_pred,labels =range(16),output_dict =True,target_names=target_names)
...
#                      color="white" if cm[i, j] > thresh else "black")
    ...
    XPatches, yPatches = createPatches(X_, y, windowSize=windowSize,removeZeroLabels=False)
    ...
    fig1 = "../images/conv_{}_no_pca_{}_{}_{}_{}_{}_{}_class_map.png".format(dataset,windowSize,testRatio,dost_,wls_alpha,wls_lambda,index)
    fig2 = "../images/conv_{}_no_pca_{}_{}_{}_no_filter_{}_class_map.png".format(dataset,windowSize,testRatio,dost_,index)
    ...
    print("scores without filter")
    print("Accuracy : {}".format(acc))
    print(cls_rep)

    print("scores with filter")
    print("Accuracy : {}".format(dost_acc))
    print(dost_cls_rep)
    pkl1 = "../excelsheets/conv_{}_no_pca_{}_{}_{}_{}_{}_{}.pkl".format(dataset,windowSize,testRatio,dost_,wls_alpha,wls_lambda,index)
    pkl2 = "../excelsheets/conv_{}_no_pca_{}_{}_{}_no_filter_{}.pkl".format(dataset,windowSize,testRatio,dost_,index)

    dost_cls_rep.to_pickle(pkl1)
    cls_rep.to_pickle(pkl2)
    fig1 = "../images/conv_{}_no_pca_{}_{}_{}_{}_{}_{}.png".format(dataset,windowSize,testRatio,dost_,wls_alpha,wls_lambda,index)
    fig2 = "../images/conv_{}_no_pca_{}_{}_{}_no_filter_{}.png".format(dataset,windowSize,testRatio,dost_,index)
...
if __name__ == "__main__":
    ...
    wa=1.2
    wl=0.8
    reportpath = "../reports/conv_{}_no_pca_{}_{}_{}_{}_{}.txt".format(xkey,ws,tr,d_,wa,wl)
    with open(reportpath,'w+') as report:
        print("""         ------------------Iteration Info : ----------------------- \n
                    Shape of X :    {}\n
                    Shape of y :    {}\n
                    numComponents : no_pca\n
                    windowSize :    {}\n
                    testRatio :     {}\n
                    use dost :      {}\n
                    wls_alpha :     {}\n
                    wls_lambda :    {}\n
            """.format(X.shape,y.shape,ws,tr,d_,wa,wl),file = report)

        wof,wf = classifier(X,y,numComponents = 200, windowSize =ws,testRatio = tr,dost_ = d_,wls_alpha=wa,wls_lambda=wl,report=report,batch_size=128,epochs=500,bands=200,target_names=target_names)
        print('\t',end='')
