...
# -*- coding: utf-8 -*-
"""
Created on Mon May 28 15:32:28 2018
...
#%%
trData = np.load("./data/DOST/train.npz").items()[0][1]
testData = np.load("./data/DOST/test.npz").items()[0][1]
trLabels = np.load("./data/DOST/trLabels.npz").items()[0][1]
testLabels = np.load("./data/DOST/testLabels.npz").items()[0][1]
#%%
...
trData = (trData-np.mean(trData,axis=0,keepdims=True))/(0.0000001+np.var(trData,axis=0,keepdims=True))
#%%
inpL = Input((256,256,1))

conv = Conv2D(16,(11,11),activation='relu')(inpL)
conv = MaxPooling2D()(conv)

conv = Conv2D(32,(11,11),activation='relu')(conv)
conv = Dropout(0.5)(conv)
conv = MaxPooling2D()(conv)

conv = Conv2D(64,(9,9),activation='relu')(conv)
conv = Dropout(0.5)(conv)
conv = MaxPooling2D()(conv)

conv = Conv2D(128,(7,7),activation='relu')(conv)
conv = Dropout(0.4)(conv)
conv = MaxPooling2D()(conv)
#lstm = LSTM(32,stateful=False,return_sequences=True)(conv)
#lstm = LSTM(64,stateful=False,return_sequences=False)(lstm)
conv = Flatten()(conv)
...
history = model.fit(x=trData[:,:,:,np.newaxis],y=trOnehot[:,:],validation_split=0.125,batch_size=16,epochs=100)
...
#%%
model.evaluate(data[testIdx][:,:,np.newaxis],testOnehot)
